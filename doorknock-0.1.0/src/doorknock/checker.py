...
class AntiBotScanner:
    ...
    def scan(self, url: str) -> ScanResult:
        ...
        try:
            ...
            raw_headers = getattr(getattr(main_resp, "raw", None), "headers", None)
            if raw_headers is not None and hasattr(raw_headers, "getlist"):
                raw_set_cookie = raw_headers.getlist("Set-Cookie")
            else:
                raw_set_cookie = []
            history = [
        ...
        if "main_body_len" not in request_meta:
            request_meta["main_body_len"] = len(body)
